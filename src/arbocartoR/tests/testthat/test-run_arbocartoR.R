...
test_that("simulation works with introduction", {
...
test_that("simulation works with introduction and human mobility", {
...
test_that("ref_norm", {
  ...
  traj_2 <- run_arbocartoR(parcels = parcels,
                           meteo = meteo,
                           n_sim = 2,
                           ref_norm = 200)

  traj_3 <- run_arbocartoR(parcels = parcels,
                           meteo = meteo,
                           n_sim = 2,
                           ref_norm = 200)
  ...
  expect_s3_class(traj[[1]], "data.frame")
  expect_length(traj, 2)
  expect_identical(traj_2[[1]]$kL %>% head, traj_3[[1]]$kL %>% head)
  expect_identical(traj_2[[1]]$kL %>% head, traj_3[[1]]$kL %>% head)
  expect_false(identical(traj[[1]]$kL %>% head, traj_3[[1]]$kL %>% head))
})
